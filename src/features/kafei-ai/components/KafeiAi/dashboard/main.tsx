...
const KafeiDashboard: React.FC = () => {
    ...
    // Right panel toggle: grid style (dots / lines)
        ...
        <DashboardLayout>
            <div className="flex flex-row w-full h-full p-5">
                ...
                <div className="w-full md:w-[70%] h-full hidden md:block p-1">
                    <ReactFlowProvider>
                        <div className="relative w-full h-full bg-transparent rounded-md">
                            {/* Top toolbar with toggle */}
                            <div className="absolute top-2 right-2 z-50 flex items-center gap-3 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1 border border-white/10">
                                ...
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input
                                        ...
                                        className="sr-only"
                                        checked={showDots}
                                        onChange={() => setShowDots((s) => !s)}
                                    />
                                    <span className={`w-11 h-6 ${showDots ? 'bg-green-500/30' : 'bg-white/20'} rounded-full shadow-inner block transition-colors`}>
                                        <span className={`block w-5 h-5 bg-white rounded-full transform transition-transform mt-0.5 ${showDots ? 'translate-x-5' : 'translate-x-0.5'}`} />
                                    </span>
                                </label>
                            </div>
...
>
  <Background
    variant={showDots ? BackgroundVariant.Dots : BackgroundVariant.Lines}
    gap={20}
    size={2}
  />
</ReactFlow>
